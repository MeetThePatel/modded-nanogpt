...
# TODO: This test is currently broken. Since this is completely untrained, I don't have confidence in this test.
# The fix is to run a small scale training loop here (maybe 50 steps). This is enough to drop the loss enough such that
# you could have some confidence in the test.
def test_function_preservation():
    ...
    test_inputs = test_targets = torch.randint(0, model_params.vocab_size, size=(TRAIN_SEQUENCE_LEN,), device="cuda")
    ...
    pre_hyperclone_logits, pre_hyperclone_loss = model(test_inputs.to(torch.int32), test_targets, get_window_size_blocks(0, N_STEPS))
    ...
    post_hyperclone_logits, post_hyperclone_loss = model(test_inputs.to(torch.int32), test_targets, get_window_size_blocks(0, N_STEPS))
